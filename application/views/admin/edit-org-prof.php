            ...
            <div class="col-lg-10 col-md-10 mx-auto">
                <div class="card">
					...
					<div class="card-body">
						...
						<div class="row">
							<?php $i=1; 
							foreach($uorg as $orow){
								echo '<div class="col-md-6"><div class="card"><div class="card-body">
								<button class="close" onClick="removeLink('.$prof[0]->id.', '.$orow->id.')"><i class="material-icons">delete</i></button>								
								<h5>'.trim($orow->title).'</h5>';
								if(!empty(${'udept_'.$i})){
									foreach(${'udept_'.$i} as $drow){
										echo '<h6>'.trim($drow->title).'</h6>';
									}
								}
								echo '</div></div></div>';
								$i++;
...
<div class="modal fade" id="ODModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog">
	  <div class="modal-content">
		...
		<form method="POST" action="<?php echo base_url('Admin/cuOrgTeacher'); ?>" id="frmTeacher">
